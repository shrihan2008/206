...
def placeNumbers():
    ...
    for row in range(0,3):
        ...
        while counter<len(numberscontainer):
            col=random.randint(0,4)

            if(col not in colist):
                colist.append(col)

    while counter<len(colist):
        colnum=colist[counter]
        numberlist=numberscontainer[str(colnum)]
        randomnumber=random.choice(numberlist)

        if(randomnumber not  in numberlist):
            numberbox=ticketGrid[row][colnum]
            numberbox.configure(text=randomnumber)
        counter+=1
